...
function renderCard(photo) {
  ...
  card.innerHTML = '<div class="photo-img"><img src="' + esc(photo.imageUrl) + '" alt="' + esc(photo.title) + '" loading="lazy">' + (photo.year ? '<span class="badge badge-year">' + esc(photo.year) + '</span>' : '') + (photo.commentCount > 0 ? '<span class="badge badge-comments"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>' + photo.commentCount + '</span>' : '') + '</div><div class="photo-info"><h3>' + esc(photo.title) + '</h3><p>' + esc(photo.description) + '</p>' + (meta ? '<div class="photo-meta">' + meta + '</div>' : '') + '</div>';
...
async function loadMore() {
  ...
  try {
    const res = await fetch('/api/photos?page=' + page + '&limit=' + LIMIT);
